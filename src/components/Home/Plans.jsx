...
const Plans = () => {
    ...
    return (
        <>
            <div className={`${theme ? "text-black" : "text-white"} flex flex-col gap-10`}>
                {
                    data.map((item,index)=>(
                ...
                >
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div className="md:w-16 w-14 h-14 md:h-16 rounded-full flex items-center justify-center bg-white/10 transition-transform group-hover:rotate-6">
                            ...
                            {
                                index ===2 ? <iframe className="" src={item.icon}></iframe> :<iframe className="w-10 h-10" src={item.icon}></iframe>
                            }
                            

                        </div>
                        <span
                            className=" text-xl flex gap-2 md:text-3xl font-inter  font-bold "
                            data-aos="fade-up-left"
                            data-aos-duration="1500"
                            data-aos-anchor-placement="top-bottom"
                        >
                            <div className="text-black/60 "><span className="line-through">{item.discountprice}</span ><span className="text-xs">$</span></div>
                            <div className="text-customTeal"><span>{item.price}</span><span className="text-xs">$</span></div>
                        </span>
                    </div>
